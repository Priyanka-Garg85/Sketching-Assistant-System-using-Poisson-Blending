...
def select_eye(request):
    if request.method == 'POST':
        global folder_index
        folder_index = request.POST.get('image_category')
        global source
        source=os.path.join(mask_path,"face_{}_{}".format(gender,int(folder_index)+1))
        print(source)
        global source_path2
        source_path2=os.path.join(source,"source.png")
        print(source_path2)
...
def select_nose(request):
    if request.method == 'POST':
        ...
        BLEND_TYPE = 2
        GRAD_MIX = True
        mask_path = os.path.join(source,"left_eye.png")
        print(mask_path)
        target_path=source_path2
        ...
        mask_path = os.path.join(source, "right_eye.png")
...
def select_mouth(request):
    if request.method == 'POST':
        ...
        BLEND_TYPE = 2
        GRAD_MIX = True
        mask_path = os.path.join(source,"nose.png")
        target_path="static/result.jpg"
        image = load_image(source_path,mask_path,target_path)
        data = preprocess(image)
        final_image = blend_image(data, BLEND_TYPE, GRAD_MIX)
...
def final(request):
    if request.method == 'POST':
        ...
        BLEND_TYPE = 2
        GRAD_MIX = True
        mask_path = os.path.join(source, "mouth.png")
        target_path="static/result.jpg"
        image = load_image(source_path,mask_path,target_path)
        data = preprocess(image)
        final_image = blend_image(data, BLEND_TYPE, GRAD_MIX)
...
def select_face_male(request):
    random_images=[os.path.join("static/face_clusters/male",i) for i in os.listdir("static/face_clusters/male") if i.endswith(("png","jpg","jpeg"))]
    # index=list(range(0,len(random_images)))
    # random_images=zip(random_images,index)
    # print(index,random_images)
    # Generate a list of tuples containing the image path and its index
    images_with_index = list(enumerate(random_images))
    global gender
    gender="m"
    return render(request, "new.html", {'raw': images_with_index, 'next_action': "form1", 'main': 'face'})
    # return render(request, "new.html",{'raw':{'image_path':random_images,'index':index},'next_action':"form1",'main':'face'})
    # return render(request, "new.html",{'raw':random_images,'next_action':"form1",'main':'face'})

def select_face_female(request):
    random_images=[os.path.join("static/face_clusters/female",i) for i in os.listdir("static/face_clusters/female") if i.endswith(("png","jpg","jpeg"))]
    images_with_index = list(enumerate(random_images))
    global gender
    gender="f"
    return render(request, "new.html", {'raw': images_with_index, 'next_action': "form1", 'main': 'face'})
    # return render(request, "new.html",{'raw':random_images,'next_action':"form1",'main':'face','index':list(range(0,len(random_images)-1))})
